...
source_format: txt
source_repo: Rag
source_path: "kb/rag-blueprint-adrs-kb/beta/ADR-006_pipeline_ingestao_fonte_base_vetorial.beta.md"
source_beta_ids:
  - "BETA-006"
...
| **Escopo**  | Pipeline de 7 etapas para ingestão de documentos na Base Vetorial |

**Referências cruzadas:**

- [[ADR-001_pipeline_geracao_conhecimento_4_fases|ADR-001]]: Pipeline de geração de conhecimento em 4 fases
- [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]]: Soberania de dados (Track A/B para embedding)
- [[ADR-003_modelo_dados_base_vetorial|ADR-003]]: Modelo de dados da Base Vetorial
- [[ADR-005_front_matter_contrato_metadados|ADR-005]]: Front matter como contrato de metadados
...
Esta ADR define o pipeline completo que transforma documentos `.md` finais (produzidos na Fase 3 da [[ADR-001_pipeline_geracao_conhecimento_4_fases|ADR-001]]) em conhecimento pesquisavel dentro da Base Vetorial. A Fase 4 da [[ADR-001_pipeline_geracao_conhecimento_4_fases|ADR-001]] mencionava "ingestao no banco vetorial" de forma resumida — este documento detalha **cada etapa** desse processo.
...
4. **Trigger por release** — O pipeline nao roda continuamente. E disparado por tags de release no repositorio knowledge-base (conforme [[ADR-001_pipeline_geracao_conhecimento_4_fases|ADR-001]], Fase 3). Apenas documentos aprovados e versionados entram na Base Vetorial.

5. **Compatibilidade com [[ADR-003_modelo_dados_base_vetorial|ADR-003]]** — O pipeline deve popular os nos e relacoes definidos no modelo de dados (Document, Chunk, System, Module, Owner, Team, etc.) respeitando constraints de unicidade e tipos de propriedades.
...
1. Para cada documento classificado como NEW ou UPDATED:
   - **Extrai o front matter**: le o bloco YAML entre `---`, converte em objeto estruturado com campos conforme [[ADR-005_front_matter_contrato_metadados|ADR-005]]
   - **Valida contra o schema** ([[ADR-005_front_matter_contrato_metadados|ADR-005]]): verifica presenca de campos obrigatorios, tipos de dados, consistencia. Se qualquer validacao falha, o documento e **rejeitado**
...
**Escolha do modelo de embedding** (conforme [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]]):
...
2. **Full-text index** no campo `Chunk.content`: busca por palavras-chave com suporte a portugues (stemming, stopwords). Complementa busca vetorial para termos exatos (codigos, siglas, nomes proprios). Ver [[ADR-007_retrieval_hibrido_agentes|ADR-007]] para busca hibrida.
...
7. **Metricas de qualidade de retrieval** (sanity check pos-ingestao): golden set de 50-100 perguntas curadas, busca vetorial top-10, Recall@10 medio, threshold >= 70%. Se cai abaixo, alerta para o time (nao bloqueia ingestao). Tendencia entre releases consecutivas para detectar degradacao. Ver [[ADR-007_retrieval_hibrido_agentes|ADR-007]] para metricas completas.
...
**Quem gera o manifest.json**: gerado automaticamente pelo pipeline de promocao (Fase 3, [[ADR-001_pipeline_geracao_conhecimento_4_fases|ADR-001]]) no momento da criacao da release tag. Contem lista de `.md` com `document_id`, hash SHA-256, `doc_type`, `last_modified` e `action` (new/updated/deleted/unchanged). Assinado pela service account do pipeline.
...
- Suporte a multiplos modelos de embedding (Track A e B da [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]])
...
| 2 (Metadados) | Governanca | Validacao de front matter ([[ADR-005_front_matter_contrato_metadados|ADR-005]]), heranca completa de metadados, indices compostos, full-text index |
...
- [[ADR-001_pipeline_geracao_conhecimento_4_fases|ADR-001]] — Pipeline de Geracao de Conhecimento em 4 Fases (este ADR detalha a Fase 4)
- [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]] — Soberania de Dados (define Track A/B para modelo de embedding)
- [[ADR-003_modelo_dados_base_vetorial|ADR-003]] — Modelo de Dados da Base Vetorial (define nos, relacoes e propriedades que este pipeline popula)
- [[ADR-005_front_matter_contrato_metadados|ADR-005]] — Front Matter como Contrato de Metadados (define schema que a Etapa 2 valida)
- [[ADR-007_retrieval_hibrido_agentes|ADR-007]] — Retrieval Hibrido e Agentes (consome a Base Vetorial populada por este pipeline)
- [[ADR-009_selecao_modelos_embedding|ADR-009]] — Selecao de Modelos de Embedding (define BGE-M3, limites de tokens e configuracao operacional)
...
- Reciprocal Rank Fusion (RRF) — ver [[ADR-007_retrieval_hibrido_agentes|ADR-007]]
...
**Data:** 22/03/2026
**Fonte:** kb/rag-blueprint-adrs-kb/beta/ADR-006_pipeline_ingestao_fonte_base_vetorial.beta.md
...
| Completude de conteúdo | 20% | 100% | Todas as seções ADR presentes. 7 etapas detalhadas individualmente. Seções de Idempotência, Re-indexação Blue/Green, Trigger, Concorrência. Tabela de referência no topo. |
...
| Linhagem rastreável | 10% | 100% | source_path aponta para beta, source_beta_ids correto (BETA-006), conversion_pipeline: promotion-pipeline-v1. |
| Clareza e estrutura | 10% | 100% | Excelente estrutura: cada etapa com Objetivo, descrição, saída. Diagramas ASCII, tabelas de implementação, seções complementares bem organizadas. |
