...
source_format: txt
source_repo: Rag
source_path: "kb/rag-blueprint-adrs-kb/beta/ADR-007_retrieval_hibrido_agentes.beta.md"
source_beta_ids:
  - "BETA-007"
...
| **Escopo**  | Busca híbrida (3 buscas + RRF + reranking) e agentes especializados por domínio |

**Referências cruzadas:**

- [[ADR-001_pipeline_geracao_conhecimento_4_fases|ADR-001]]: Pipeline de geração de conhecimento em 4 fases
- [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]]: Soberania de dados (Track A/B)
...
- [[ADR-005_front_matter_contrato_metadados|ADR-005]]: Front matter obrigatório
- [[ADR-006_pipeline_ingestao_fonte_base_vetorial|ADR-006]]: Pipeline de ingestão
- [[ADR-009_selecao_modelos_embedding|ADR-009]]: Seleção de modelos de embedding e reranking
...
Esta ADR atua na **Fase 4 — Geração do RAG (Camada Ouro)**. O pipeline de 4 fases definido na [[ADR-001_pipeline_geracao_conhecimento_4_fases|ADR-001]] estabelece:
...
A Base Vetorial, populada pelo pipeline definido na [[ADR-006_pipeline_ingestao_fonte_base_vetorial|ADR-006]], contém nós :Document com metadados ricos, nós :Chunk com embeddings vetoriais e conteúdo textual, grafo de relações e índices vetoriais, full-text e compostos.
...
**Dual-track (conforme [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]] e [[ADR-009_selecao_modelos_embedding|ADR-009]]):**
...
**Níveis de confidencialidade (conforme [[ADR-004_seguranca_classificacao_dados|ADR-004]]):**
...
Um LLM reformula queries vagas em versões mais precisas. Respeita a [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]] (soberania):
...
- **Grafo incompleto:** mitigação via verificação de consistência ([[ADR-006_pipeline_ingestao_fonte_base_vetorial|ADR-006]], Etapa 7)
...
**Data:** 22/03/2026
**Fonte:** kb/rag-blueprint-adrs-kb/beta/ADR-007_retrieval_hibrido_agentes.beta.md

| Critério | Peso | Nota | Comentário |
|----------|------|------|------------|
| Front matter rico | 25% | 96% | Todos os campos obrigatórios presentes. 12 tags e 5 aliases (mínimos atendidos). Corrigido: `status: accepted` alterado para `status: approved` (valor válido conforme schema). |
| Completude de conteúdo | 20% | 100% | Todas as seções ADR completas. Busca híbrida detalhada (3 buscas + RRF + Reranking). Filtragem temporal, segurança pré-retrieval, processamento de query, agentes especializados, feedback loop. Tabela de referência no topo. |
...
| Linhagem rastreável | 10% | 100% | source_path aponta para beta, source_beta_ids correto (BETA-007), conversion_pipeline: promotion-pipeline-v1. |
| Clareza e estrutura | 10% | 98% | Estrutura muito boa com tabelas comparativas, seções bem delimitadas. Referências acadêmicas incluídas. Pequena oportunidade: seção de Agentes poderia incluir exemplo de system prompt. |

**Score:** 99.0% — APROVADO para ingestão

**Por que não é 100%:** (1) Campo `status` continha valor `accepted` não previsto no schema — corrigido para `approved` (-4% no front matter rico). (2) Seção de Agentes Especializados poderia detalhar exemplos de system prompts para cada agente (-2% clareza).
<!-- QA-MD: fim -->
