...
source_format: txt
source_repo: Rag
source_path: "kb/rag-blueprint-adrs-kb/beta/ADR-009_selecao_modelos_embedding.beta.md"
source_beta_ids:
  - "BETA-009"
...
| **Decisor** | fabio |
| **Escopo** | Modelos de Embedding |

**Referências Cruzadas:**

- **Depende de:** [[ADR-001_pipeline_geracao_conhecimento_4_fases|ADR-001]], [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]]
- **Relaciona-se:** [[ADR-006_pipeline_ingestao_fonte_base_vetorial|ADR-006]], [[ADR-007_retrieval_hibrido_agentes|ADR-007]], [[ADR-008_governanca_ciclo_vida_rollback|ADR-008]]

## Sumário

Este ADR define a seleção, configuração e operação dos modelos de embedding para a **Trilha B (On-Premises)** do RAG Corporativo. A Trilha B, estabelecida pela [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]], é obrigatória para documentos classificados como "restricted" ou "confidential" — dados que NÃO PODEM sair da infraestrutura controlada pela organização.
...
- **Compatibilidade com retrieval:** deve ser compatível com a estratégia híbrida da [[ADR-007_retrieval_hibrido_agentes|ADR-007]]

### Requisito de Soberania ([[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]], Trilha B)
...
**Integração no pipeline de retrieval ([[ADR-007_retrieval_hibrido_agentes|ADR-007]]):**
...
**Vetores das duas trilhas são INCOMPATÍVEIS.** Índices separados por trilha, conforme [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]].
...
### Thresholds por Fase (alinhados com [[ADR-008_governanca_ciclo_vida_rollback|ADR-008]])
...
| Alternativa | Motivo |
|-------------|--------|
| OpenAI text-embedding-3-small/large | Cloud-only, viola soberania ([[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]]) para restricted/confidential |
...
- [[ADR-002_soberania_dados_cloud_vs_onprem|ADR-002]]: Soberania de dados — Trilha Cloud vs. On-Premises
- [[ADR-006_pipeline_ingestao_fonte_base_vetorial|ADR-006]]: Pipeline de ingestão
- [[ADR-007_retrieval_hibrido_agentes|ADR-007]]: Retrieval híbrido e agentes especializados
- [[ADR-008_governanca_ciclo_vida_rollback|ADR-008]]: Governança — Papéis, ciclo de vida e rollback
...
**Data:** 22/03/2026
**Fonte:** kb/rag-blueprint-adrs-kb/beta/ADR-009_selecao_modelos_embedding.beta.md

| Critério | Peso | Nota | Comentário |
|----------|------|------|------------|
| Front matter rico | 25% | 95% | Todos os campos obrigatórios presentes. Campo `status` corrigido de "accepted" para "approved" (valor válido no schema). Campo `converted_at` presente como extra. |
| Completude de conteúdo | 20% | 98% | Todas as seções ADR presentes: Contexto, Decisão, Alternativas Descartadas, Consequências, Implementação, Referências. Tabela de referência e cross-references completas. |
...
| Clareza e estrutura | 10% | 97% | Headings hierárquicos, tabelas bem formatadas, código Python ilustrativo, seções de implementação por fase. |

**Score:** 98.1% — APROVADO para ingestão

**Por que não é 100%:** Campo `status` original era "accepted" (valor fora do schema, corrigido para "approved"). Pequena penalização residual no front matter pela necessidade de correção.
<!-- QA-MD: fim -->
