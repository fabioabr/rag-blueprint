...
sources:
  - type: "txt"
    origin: "Arquitetura/rag-blueprint/kb/rag-blueprint-adrs-kb/1 - draft/ADR-005_front_matter_contrato_metadados.txt"
    captured_at: "2026-03-21"
tags: [front-matter, metadados, contrato, validacao, yaml, obsidian, schema, linhagem]
aliases: ["Contrato de Metadados", "Front Matter YAML", "Schema de Validacao Front Matter"]
status: approved
...
- **release_version**: versao da release que disparou a ingestao (formato semver `vN.N.N`). Preenchido automaticamente pelo pipeline de ingestao ([[BETA-006]]).
...
|----------|------|------|------------|
| Front matter leve | 25% | 100% | Todos os campos obrigatórios presentes e válidos. id BETA-005 (regex ok), title 42 chars, domain lowercase, confidentiality enum válido, sources com type/origin/captured_at, 8 tags lowercase, status approved, datas válidas, aliases presente. Nenhum campo de governança indevido. |
| Completude de conteúdo | 25% | 98% | Todas as seções principais preservadas: Contexto, Decisão (Partes A-E), Schema de Validação (Parte C), Conversion Quality (Parte D), Compatibilidade Obsidian (Parte E), Alternativas Descartadas, Consequências, Implementação, Referências. Tabelas e exemplos YAML preservados. Leve condensação em relação ao draft (esperado na conversão). |
| Blocos LOCKED | 15% | 100% | Bloco LOCKED presente (linha 59-298) protegendo Parte A e Parte B — decisões-chave de front matter leve e rico. Corretamente aberto e fechado. |
| Wikilinks | 10% | 100% | Referências cruzadas em formato [[BETA-NNN]] corretas: BETA-001, BETA-002, BETA-003, BETA-004, BETA-006, BETA-007, BETA-009, BETA-010, BETA-011. Nenhum wikilink no front matter. |
| Compatibilidade Obsidian | 10% | 100% | YAML válido entre marcadores ---. Tags como array YAML. Aliases como array. Campos tipados corretamente. |
| Clareza e estrutura | 15% | 98% | Headings hierárquicos claros (H1 > H2 > H3 > H4). Tabelas bem formatadas. Exemplos YAML legíveis. Seções logicamente organizadas por Partes (A-E). |

**Score:** 99.2% — APROVADO para promoção

**Por que não é 100%:** Leve condensação de conteúdo em relação ao draft original (ex: justificativas detalhadas de "por que existe" para cada campo foram resumidas). Impacto mínimo — a essência e decisões estão integralmente preservadas.
<!-- QA-BETA: fim -->
