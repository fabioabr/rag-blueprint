...
sources:
  - type: "txt"
    origin: "Arquitetura/rag-blueprint/kb/rag-blueprint-adrs-kb/1 - draft/ADR-004_seguranca_classificacao_dados.txt"
    captured_at: "2026-03-21"
tags: [seguranca, classificacao-dados, lgpd, filtro-pre-retrieval, autenticacao, autorizacao, prompt-injection]
aliases: ["ADR-004", "Seguranca RAG", "Classificacao de Confidencialidade"]
status: approved
...
Este ADR define a estrategia de seguranca e classificacao de dados para o RAG Corporativo, baseada em **6 pilares complementares**: (1) classificacao de dados, (2) filtro pre-retrieval inviolavel, (3) autenticacao evolutiva, (4) autorizacao evolutiva, (5) protecao contra prompt injection, e (6) conformidade LGPD e regulatoria. Cada pilar endereca uma camada diferente do problema e juntos formam defesa em profundidade.
...
- **(c)** [[BETA-002]] define que dados RESTRICTED e CONFIDENTIAL devem ser processados on-premise — pipelines de processamento separados por classificacao
...
|------|----------|-------------|
| **1-MVP** | Campo confidentiality no front matter, filtro pre-retrieval basico (WHERE), API Key, log basico de queries | [[BETA-001]] Fase 1 |
| **2** | 4 perfis RBAC, mapeamento perfil→niveis, pipeline de anonimizacao (NER + regex), validacao de output basica | [[BETA-001]] Fase 2 |
| **3** | JWT via IdP (OIDC), migracao API Key→JWT, ABAC basico (dominio + confidentiality), sanitizacao avancada, rate limiting | [[BETA-001]] Fase 3 + IdP configurado |
| **4** | ABAC completo (atributos de contexto), monitoramento de anomalias, auditoria 5 anos, procedimento de exclusao LGPD testado, comunicacao ao BACEN | [[BETA-001]] Fase 4 |
...
|----------|------|------|------------|
| Front matter leve | 25% | 100% | id BETA-004 valido, title descritivo (51 chars), domain lowercase, confidentiality enum valido, sources com type/origin/captured_at (conversion_quality ausente mas opcional), 7 tags, status approved, aliases array (3), datas ISO validas. Nenhum campo de governanca presente. |
| Completude de conteudo | 25% | 92% | Secoes principais preservadas: Sumario, Contexto (alinhamento com ADR-001), Decisao (6 pilares: classificacao, filtro pre-retrieval, autenticacao, autorizacao, prompt injection, LGPD), Alternativas (5), Consequencias (positivas/negativas/riscos), Implementacao, Referencias. O .txt de 1711 linhas possui detalhamento adicional — principalmente nos pilares 3/4 (autenticacao/autorizacao com mais exemplos de fluxo), pilar 5 (exemplos concretos de prompt injection) e pilar 6 (pipeline de anonimizacao com mais detalhes). |
| Blocos LOCKED | 15% | 100% | Bloco LOCKED presente (linhas 53-62) protegendo a tabela de 4 niveis de classificacao — a decisao mais critica de seguranca. Corretamente aberto e fechado. |
| Wikilinks | 10% | 95% | Usa formato [[BETA-001]], [[BETA-002]], [[BETA-003]] corretamente. Referencias a ADR-006, ADR-007, ADR-008 sem wikilink — aceitavel pois esses ADRs ainda nao possuem versao beta, mas idealmente deveriam ser convertidos para [[BETA-NNN]] quando existirem. Sem wikilinks no front matter. |
| Compatibilidade Obsidian | 10% | 100% | YAML entre --- valido, tags como array YAML, aliases como array YAML. |
| Clareza e estrutura | 15% | 95% | Organizacao por pilares numerados e clara e eficaz. Tabelas de classificacao, perfis RBAC e KBs bem formatadas. Code block Python para SecureRetriever. Numeracao hierarquica consistente. Secoes de implementacao com faseamento claro. |

**Score:** 96.8% — APROVADO para promocao

**Por que nao e 100%:** (1) Completude: o .txt original (1711 linhas) possui mais detalhamento nos pilares 3-6, incluindo exemplos de fluxo JWT, exemplos de prompt injection com payloads, e pipeline de anonimizacao com mais granularidade. (2) Wikilinks: referencias a ADR-006/007/008 nao usam formato [[BETA-NNN]] — justificavel pois esses betas ainda nao existem, mas sera necessario atualizar quando forem criados. (3) conversion_quality ausente no sources (opcional mas inconsistente com BETA-001 e BETA-002).
<!-- QA-BETA: fim -->
